...
<c:if test="${param.referName ne 'commited'}">
	<th>${userContext.localeMap['change_request.commited']}</th>
</c:if>
...
<c:if test="${param.referName ne 'commited'}">	<td contenteditable='true' class='edit-value'  propertyToChange='commited' storedCellValue='${item.commited}' prefix='${ownerBeanName}Manager/updateChangeRequest/${result.id}/${item.id}/'>${item.commited}</td>
