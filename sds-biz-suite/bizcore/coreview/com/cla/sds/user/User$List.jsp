...
<c:if test="${param.referName ne 'joinTime'}">
	<th>${userContext.localeMap['user.join_time']}</th>
</c:if>
<c:if test="${param.referName ne 'company'}">
	<th>${userContext.localeMap['user.company']}</th>
</c:if>
<c:if test="${param.referName ne 'title'}">
	<th>${userContext.localeMap['user.title']}</th>
</c:if>
...
</c:if><c:if test="${param.referName ne 'joinTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='joinTime' storedCellValue='${item.joinTime}' prefix='${ownerBeanName}Manager/updateUser/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd" value="${item.joinTime}" /></td>
</c:if><c:if test="${param.referName ne 'company'}">
	<td class="select_candidate_td"
			data-candidate-method="./userManager/requestCandidateCompany/${ownerBeanName}/${item.id}/"
			data-switch-method="./userManager/transferToAnotherCompany/${item.id}/"
...
<c:if test="${param.referName ne 'title'}">	<td contenteditable='true' class='edit-value'  propertyToChange='title' storedCellValue='${item.title}' prefix='${ownerBeanName}Manager/updateUser/${result.id}/${item.id}/'>${item.title}</td>
</c:if>
				<td>
